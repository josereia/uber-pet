...
vector<vector<string>> content = PetRepository::find_all();
...
void on_update() {
  auto results = PetRepository::find_all();
  content = results;
  filtered = results;
}
...
Component main() {
  ...
  auto renderer = Renderer(
      container, [search_input, new_button, update_button, delete_button] {
        vector<string> table_header = {
            "ID", "Nome", "Raça", "Sexo", "Tamanho", "Peso(Kg)", "Observação",
        };
  ...
  renderer |= Modal(PetWidgets::CreateModal::create_modal(on_cancel, on_update),
                    &create_modal_shown);

  renderer |= Modal(PetWidgets::UpdateModal::update_modal(on_cancel, on_update),
                    &update_modal_shown);

  renderer |= Modal(PetWidgets::DeleteModal::delete_modal(on_cancel, on_update),
                    &delete_modal_shown);
